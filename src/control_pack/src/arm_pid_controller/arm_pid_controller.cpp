...
controller_interface::CallbackReturn ArmPidController::on_init() {
    try {
        ...
        reference_interface_types_ = auto_declare<std::vector<std::string>>(
            "reference_interfaces", {"position", "velocity", "effort", "kp", "ki", "kd"});
...
controller_interface::return_type ArmPidController::update_and_write_commands(
    ...
    for (size_t i = 0; i < joint_names_.size(); ++i) {
        const double desired_position = get_reference_value(i, "position", 0.0);
        const double desired_velocity = get_reference_value(i, "velocity", 0.0);
        const double feedforward_effort = get_reference_value(i, "effort", 0.0);
        const double kp = get_reference_value(i, "kp", default_kp_[i]);
        const double ki = get_reference_value(i, "ki", default_ki_[i]);
        const double kd = get_reference_value(i, "kd", default_kd_[i]);
...
std::vector<hardware_interface::CommandInterface> ArmPidController::on_export_reference_interfaces() {
    ...
    interfaces.reserve(joint_names_.size() * reference_interface_types_.size());
